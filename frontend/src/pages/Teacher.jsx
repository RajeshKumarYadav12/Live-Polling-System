...
function Teacher() {
  ...
  useSocket({ role: "teacher", enabled: true, onPollEnded: loadPollHistory });
  ...
  return (
    <div className="page-container">
      <button className="view-history-btn" onClick={() => setIsHistoryModalOpen(true)}>
            ...
            <>
              <div className="card" style={{ marginBottom: "2rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: "1rem" }}>
                  ...
                  <button className="btn btn-secondary" onClick={handleEndPoll} style={{ fontSize: "0.9rem", padding: "0.5rem 1rem" }}>
                    End Poll
                  </button>
                </div>
                <Timer duration={currentPoll.duration} onTimeUp={handleTimeUp} />
              </div>

              <ResultChart results={currentPoll.results} totalResponses={currentPoll.totalResponses} />
            </>
          )}
        </div>
      </div>

      <PollHistory polls={pollHistory} isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} />

      <ChatButton onClick={() => setIsChatOpen(!isChatOpen)} />
      <ChatPopup isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} userRole="teacher" />
    </div>
  );
}
